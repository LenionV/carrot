...
function loadGame(btn) {
		...
		s("#page-2 button").addEventListener("click", function() {
				...
				if (input === randomNumber) {
						...
						if (tries == 1) {
								s("#page-3>p").innerHTML = "ВАУ! Потребовалось всего ... 1 попытка!";
								s(".tip").innerHTML = "Впечатляет!";
						} else if (tries < 5) {
								s(".tip").innerHTML = "Хорошо!";
						}
				} else if (input < randomNumber) {
						s(".response").innerHTML = "Мало!";
				} else
				if (input > randomNumber) {
						s(".response").innerHTML = "Много!";
				} else {
						s(".response").innerHTML = "Error!";
						return;
				}
